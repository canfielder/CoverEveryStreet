...
```{r import_data}
file_dir <- "./../data"
file_name <- "network_03_parallel_edges_identified_2021-08-24.RDS"
...
## Generate Parallel Edge Pairs 
```{r}
network.df

parallel_edge_pairs <- network.df %>% 
...
## Create Table of All Segments Containing Parallel Edges
```{r}
segment_breakdown <- parallel_edge_pairs %>% 
  group_by(segment_id) %>%
...
segment_breakdown %>% arrange(desc(n_parallel_edges))
```

## Summary Statistics - Unique Segments
```{r}
...
# Generate All Sets of  Parallel Segment Pairs
...
verifyAdjacentSegments <- function(segment_1, segment_2){
  ...
  segment_1_adj <- segment_map[segment_1][[1]]
...
s3 <- intersect(segment_map[s1][[1]], segment_map[s2][[1]])

length(segment_map)

length(unique(network.df$segment_id))
```
      ...
      minimize_distance = 
        purrr::pmap(
          .l = list(
            ...
            lat_intersect = short_intersect_lat,
            x_points      = x_points
             ), 
          .f =  minimumDistance
           )
