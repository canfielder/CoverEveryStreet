...
# Purpose
For streets that are represented by two parallel edges. The intent is to ensure that both edges are 
split at the same point if there is only an intersection on one of the edges (T-intersection)
...
# Important Edge/Segments
segment_id_1 <-"27e3erinva"
edge_id_1 <- 683214
segment_id_2 <- "wha36dme2n"
edge_id_2 <- 677591
segment_id_3 <- "fqdf73jxcj"
edge_id_3 <- 677581
segment_id_4 <- "0qnthk73jk"
edge_id_4 <- 677565
...
## Verify
```{r}
network_subset.sf <- makeLinestringVector(network_subset.df)

network_subset.sf <- network_subset.sf %>% 
  mutate(
    highlight = case_when(
      segment_id == segment_id_1 ~ "long",
      segment_id == segment_id_2 ~ "short_1",
      segment_id == segment_id_3 ~ "short_2",
      segment_id == segment_id_4 ~ "intersect",
      # edge_id == edge_id_1 ~ "a",
      # edge_id == edge_id_2 ~ "b",
      # edge_id == edge_id_3 ~ "c",
      # edge_id == edge_id_4 ~ "d",
